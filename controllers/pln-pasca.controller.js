...
class PLNPascaController {
  static async inquiry(req, res, next) {
    ...
    // const validateDatas
    const insertedDatas = await PLNPascaService.insertDatas(datas);
    const dataTransactions = await PaymentService.processTransactions(
      insertedDatas,
      PaymentService.inqTransactions
    );

    const newDatas = await PLNPascaService.updateDatas(dataTransactions);
  ...
  static async payment(req, res, next) {
    ...
    const findDatas = await PLNPascaService.findByIds(datas);
    const dataTransactions = await PaymentService.processTransactions(
      findDatas,

...
class PostpaidController {
    static async inquiry(req, res, next) {
        ...
        const dataTransactions = await PaymentService.processTransactions(
            insertedDatas,
            PaymentService.inqPostpaidTransactions
        );
        const newDatas = await TransactionService.updateDatas(dataTransactions);
        const generateExcel = await ExcelHelper.writeInquiryPostpaidToExcel(newDatas)
        res.setHeader(
            "Content-Type",
    ...
    static async payment(req, res, next) {
        ...
        const dataTransactions = await PaymentService.processTransactions(
            findDatas,
            PaymentService.payPostpaidTransactions
        );
...
module.exports = PostpaidController